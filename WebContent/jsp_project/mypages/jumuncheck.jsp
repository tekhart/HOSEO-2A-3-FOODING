...
<body id="body">
	...
	<div id="maindiv">
		...
		<table class="jumuntb" border="1">
			<tr class="jumuntr1">
				...
				<td>제품 이름</td>
				<td>주문 개수</td>
				<td class="tooltip">가격 <img src="../img/what2.png" width="25px"
					height="25px" style="vertical-align: middle;"> <span
					class="tooltiptext"> 할인 / 사용 포인트가 적용된 금액입니다 </span>
				</td>
				<td>비고</td>
			</tr>
			<%
			...
			<tr>
				<%
					if (articleint == 0) {
				%>
				<td rowspan="<%=DataArticleList.size() + 1%>"><%=article.getBuyId()%>
				</td>
				<td rowspan="<%=DataArticleList.size()%>"><%=article.getProductName()%>
					<%
						if (DataArticleList.size() != 1) {
					%> 외 <%=DataArticleList.size() - 1%>개의
					제품 <%
						}
					%></td>
				<%
				...
				<td>
					<div
						style="background-image:url('<%=article.getProductThumb()%>');background-size:cover;background-position:center;width:290px;height:163px;"></div>
				</td>
				<td><%=article.getProductName()%></td>
				<td><%=article.getProductCount()%> 개</td>
				<td>
					<%
						if (article.getDiscountRate() != 0) {
					%> <%=article.getDiscountRate()%>%
					<%=article.getPrice()%>원<br> <%
 	}
 %> <%=realprice%>원
				</td>
				<td <%if (article.getAccountId() > 0 && article.getAccountId() < 6) {%>
 ...
 %></td>
			...
			%>
			<tr>
				<td>결제 일자 <br> <%=sdf.format(article.getBuydate())%>
				</td>
				<td colspan="3">사용포인트 : <%=article.getPointused()%>포크
				</td>
				<td>총 가격 : <%=totalprice%>원
				</td>
